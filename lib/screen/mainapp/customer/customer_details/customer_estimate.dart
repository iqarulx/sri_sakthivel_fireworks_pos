...
import '../../estimate/estimate_details.dart';
...
class _CustomerEstimateState extends State<CustomerEstimate> {
  ...
  Future getEnquiryInfo() async {
    try {
      ...
      if (cid != null) {
        var enquiry = await FireStoreProvider().getEstimateCustomer(cid: cid, customerID: widget.customerID!);
      ...
      await EnquiryExcel(enquiryData: enquiryList, isEstimate: true).createCustomerExcel().then((value) async {
        if (value != null) {
          ...
          await DownloadFileOffline(
            ...
            snackBarCustom(context, true, "Estimate Excel Download Successfully");
          }).catchError((onError) {
            Navigator.pop(context);
